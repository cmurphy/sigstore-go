...
var (
	rootHash         = []byte("unreadable binary")
	expectedRootHash = []byte("756e7265616461626c652062696e617279")
)
...
func TestNewTlogEntry(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			newEntry, err := NewTlogEntry(&tle)
			if test.expectErr {
				assert.Error(t, err)
				return
			}
			assert.NoError(t, err)
			assert.Equal(t, newEntry.tle.InclusionProof.RootHash, expectedRootHash)
		})
	}
}
